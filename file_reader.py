def get_topic_from_file():
    with open('input_to_essay_v1\\resources\\index.txt', 'r') as index_file:
        current_index = int(index_file.readline().strip())

    with open('input_to_essay_v1\\resources\\topics.txt', 'r', encoding='utf-8') as topics_file:
        topics = topics_file.readlines()
        current_topic = topics[current_index - 1].strip() if 1 <= current_index <= len(topics) else None

    new_index = current_index + 1

    with open('input_to_essay_v1\\resources\\index.txt', 'w') as index_file:
